...
class TestNode(Runnable):
    ...
    def is_cleanup_ready(self, worker: TestWorker) -> bool:
        ...
        if not self.is_flat() and worker.id not in self.params["name"]:
            raise RuntimeError(f"Cannot consider cleanup readiness of node {self} which "
    ...
    def pick_child(self, worker: TestWorker) -> "TestNode":
        ...
        if self.is_shared_root():
            ...
            previous_nodes = [n for n in sorted_nodes if worker.id in n._picked_by_setup_nodes.get_workers()]
            if len(previous_nodes) == 0:
                should_reverse = True
                previous_nodes = [n for n in sorted_nodes if len(n._picked_by_setup_nodes.get_workers()) > 0]
            score = lambda x: sum([TestNode.variant_distance(x.params["name"], n.params["name"]) for n in previous_nodes])
            sorted_nodes = sorted(sorted_nodes, key=lambda n: score(n), reverse=should_reverse)
