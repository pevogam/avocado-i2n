...
class MockDriver(unittest.TestCase):
    ...
    def mock_show(self, state_names, state_type, root_exists=True):
        ...
        if backend in ["lvm", "qcow2", "qcow2ext", "ramfile"]:
            backend_class = lvm.LVMBackend if backend == "lvm" else qcow2.QCOW2ExtBackend
            backend_class = qcow2.QCOW2Backend if backend == "qcow2" else backend_class
            backend_class = ramfile.RamfileBackend if backend == "ramfile" else backend_class
...
class StatesBoundaryTest(Test):
    ...
    def setUp(self):
        ...
        self.run_params["unset_switch"] = "hard"
        self.run_params["show_state"] = ".+"
    ...
    def test_check_vm_qcow2(self):
        ...
        backend = "qcow2vt"
        backend_type = self._prepare_driver_from_backend(backend)
        # test for two images rather than one
        self.run_params["images_vm1"] = "image1 image2"
        self.run_params["image_name_image1_vm1"] = "vm1/image1"
        self.run_params["image_name_image2_vm1"] = "vm1/image2"
        # keep a passive precondition behavior for this test
    ...
    def test_check_vm_ramfile(self):
        ...
        for image_format in ["qcow2", "raw", "something-else"]:
            with self.subTest(f"Test root checking with ramfile using image format {image_format}"):
                ...
                # TODO: is this still tested? show and initialize are both mocked
                # TODO: initialize and finalize for vm backends are currently failing
                with self.driver.mock_show([], backend_type, True) as driver:
    ...
    @skip("need root methods for ramfile + unification with qcow2vt")
...
class StatesSetupTest(Test):
    ...
    def test_show_multiobj(self):
        ...
        self.assertEqual(call_params[2]["vms"], "vm1")
        self.assertEqual(call_params[2]["object_name"], "net1/vm1")
        self.assertEqual(call_params[2]["object_type"], "nets/vms")
        self.assertEqual(call_params[3]["vms"], "vm2")
        self.assertEqual(call_params[3]["images"], "image21")
        self.assertEqual(call_params[3]["object_name"], "net1/vm2/image21")
        self.assertEqual(call_params[3]["object_type"], "nets/vms/images")
        self.assertEqual(call_params[4]["vms"], "vm2")
        self.assertEqual(call_params[4]["object_name"], "net1/vm2")
        self.assertEqual(call_params[4]["object_type"], "nets/vms")
        self.assertTrue(exists)
